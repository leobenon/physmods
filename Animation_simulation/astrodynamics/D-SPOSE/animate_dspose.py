...
ax.set_xlabel(r"$X_{BRF}$")
ax.set_ylabel(r"$Y_{BRF}$")
ax.set_zlabel(r"$Z_{BRF}$")
...
ani_axis_rotation = FuncAnimation(
    fig,
    rotate_camera,
...
# save if desired
if args.save:
    ani_axis_rotation.save(
        outdir / f"{args.prefix}_body_axis_tip_rotation.mp4",
        fps=args.fps,
        dpi=args.dpi
    )
...
def rotate_camera(frame):
    ax.view_init(
        elev=25,          # fixed elevation
        azim=frame        # rotating azimuth
    )
    return fig,
...
ani_rot = FuncAnimation(
    fig,
    rotate_camera,
    frames=np.linspace(0, 360, 240),   # one full rotation
    interval=50,
    blit=False
)
